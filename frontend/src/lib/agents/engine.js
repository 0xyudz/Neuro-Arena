...
class AgentEngine {
    ...
    tick() {
        this.round++;
        const agents = AgentRegistry.getAgents().filter((a) => a.runtime?.state === 'active');
        if (agents.length < 2)
            return;
        ...
        try {
            ...
            [agentA, agentB].forEach((agent) => {
                ...
                AgentRegistry.updateAgent(agent.id, updates);
            });
            // Safe name access
            const nameA = agentA.metadata?.name || agentA.name || 'Agent A';
            const nameB = agentB.metadata?.name || agentB.name || 'Agent B';
            // Log battle
            AgentLog.add({
                agentId: agentA.id,
                agentName: nameA,
