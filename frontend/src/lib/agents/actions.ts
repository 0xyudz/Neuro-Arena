...
const BASE_OUTCOMES: Record<AgentAction, { success: number; win: number; lose: number }> = {
  attack: { success: 0.6, win: 5, lose: -3 },
  defend: { success: 0.9, win: 2, lose: -1 },
  adapt: { success: 0.7, win: 4, lose: -2 },
};

export function executeAction(agent: Agent, action: AgentAction): ActionResult {
  ...
  const reasons: Record<AgentAction, { true: string; false: string }> = {
...
export function calculatePrice(score: number, stake: number): number {
  const base = 0.01;
