...
function formatAgent(agent: any) {
  return {
    ...
    runtime: {
      ...
      history: agent.battleLogs?.map((log: any) => ({
    ...
    token: agent.price > 0 ? {
      ...
      transactions: agent.transactions?.map((tx: any) => ({
        type: tx.type,
        amount: tx.amount,
        price: tx.price,
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const {
      ...
      strategy,
      imageUrl,
      launchRealToken = false, // ✅ NEW: Flag untuk launch real token
    } = req.body;
    ...
    if (launchRealToken && process.env.BAGS_API_KEY) {
      ...
      const launchResult = await bagsService.launchToken({
        name,
        symbol: tokenSymbol,
...
router.post('/quote', async (req: Request, res: Response) => {
  try {
    ...
    const result = await bagsService.getQuote({
      inputMint,
      outputMint,
      amount,
    });
...
router.post('/swap/build', async (req: Request, res: Response) => {
  try {
    ...
    const result = await bagsService.buildSwap({
      inputMint,
      outputMint,
      amount,
      userPublicKey,
