...
function calcAttack(agent: Agent): number {
...
function calcDefense(agent: Agent): number {
  const { consistency } = agent.attributes || { consistency: 0.5 };
  return consistency * 1.5 * (0.9 + Math.random() * 0.2);
}

function applyStrategy(agent: Agent, baseValue: number): number {
  const strat = agent.metadata?.strategy;
  if (strat === 'aggressive') return baseValue * 1.15;
  if (strat === 'defensive') return baseValue * 0.95;
  return baseValue;
}

export function runBattle(agentA: Agent, agentB: Agent): BattleResult {
...
export function updateAgentAfterBattle(agent: Agent, result: BattleResult, isWinner: boolean): Partial<Agent> {
...
export function boostAgent(agent: Agent): Partial<Agent> {
  const scoreBoost = 10;
